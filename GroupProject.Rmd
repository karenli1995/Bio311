...
huebert.tfs <- huebert_TF %>% 
  dplyr::select(-Gene.Symbol, -Category, -Chromosome, -Loci.Start, -Loci.End, -Strand, -Distance.to.TSS, -PeakID, -Significantly.Enriched.over.WCE) %>% 
  dplyr::rename(target = Gene.ID)
```

```{r harbison TF binding data, echo=FALSE}
harbison_TF <- fread("harbison-2004-TFbinding-other.txt")
harbison_TF2 <- fread("harbison-2004-TFbinding-YPD.txt")
```
