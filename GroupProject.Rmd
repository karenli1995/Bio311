...
V(g1)$size <- degree(g1) * 0.5
l1 <- layout_with_fr(g1)
plot(g1, layout = l1, edge.arrow.size = .15, 
     vertex.color = "gray", vertex.label.cex = 0.4, 
     mark.groups = c("MSN2", "MSN4"), mark.col = c("red", "green"), 
     rescale = TRUE)

hist(degree(g1, mode = "out"), breaks = 1:vcount(g1)-1)
table(degree(g1, mode = "out"))
```
...
plot(g2, layout = l2, edge.arrow.size = .1, 
     vertex.color = "gray", vertex.label.cex = 0.4, 
     mark.groups = c("MSN2"), mark.col = c("red"), rescale = TRUE)
